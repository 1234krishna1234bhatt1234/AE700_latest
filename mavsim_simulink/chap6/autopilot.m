...
%     NN = NN+4;
    ...
    beta_c  = 0; % Commanded sideslip is typically zero
    delta_r = sideslip_with_rudder(beta_c, beta, flag, AP); % REPLACED YAW DAMPER
...
function phi_c_sat = course_with_roll(chi_c, chi, flag, AP)
    ...
    if flag==0 || isempty(integrator), integrator = 0; end
    error = chi_c - chi;
...
function delta_r_sat = sideslip_with_rudder(beta_c, beta, flag, AP)
    persistent integrator;
    if flag==0 || isempty(integrator), integrator = 0; end
    error = beta_c - beta;
    integrator = integrator + (AP.Ts/2)*error;
    delta_r = AP.sideslip_kp*error + AP.sideslip_ki*integrator;
    delta_r_sat = sat(delta_r, AP.delta_r_max, -AP.delta_r_max);
end
