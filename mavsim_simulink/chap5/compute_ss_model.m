...
B_lon = [ B(4,1),  B(4,4);   % u_dot equation
          B(6,1),  B(6,4);   % w_dot equation
          B(11,1), B(11,4);  % q_dot equation
          0,       0;        % theta_dot equation
          0,       0 ];      % h_dot equation
...
A54_lat = q_star*cos(phi_star)*sec(theta_star) - r_star*sin(phi_star)*sec(theta_star);
...
disp('State-space matrices (A_lon, B_lon, A_lat, B_lat).');
display(A_lon);
display(A_lat);
display(B_lon);
display(B_lat);